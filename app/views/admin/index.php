...
$allowedDisplayPeriods = $allowedDisplayPeriods ?? [24, 48, 72, 168];
$admins = $admins ?? [];
...
<div class="max-w-7xl mx-auto px-4 py-6 sm:p-6">
    ...
    <section class="bg-slate-800 rounded-xl p-5 mb-6">
        ...
        <form method="post" action="/admin/settings" class="flex flex-col md:flex-row md:items-end gap-4">
            <div>
                ...
                >
                    <?php foreach ($allowedDisplayPeriods as $period): ?>
    ...
    <section class="bg-slate-800 rounded-xl p-5 mb-6">
        ...
        <form method="post" action="/admin/endpoints" class="space-y-4">
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
                <div class="lg:col-span-2">
                    ...
                    <input
                            ...
                            class="w-full h-[42px] rounded-lg bg-slate-900 border border-slate-700 px-3 py-2 text-slate-100 outline-none focus:border-emerald-400"
                    >
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
                <div class="lg:col-span-2">
                    <div class="h-full pt-6">
                        <label class="flex items-center justify-center gap-2 h-[42px] rounded-lg bg-slate-900 border border-slate-700 cursor-pointer">
                            <input
                                    type="checkbox"
                            ...
                            <span class="relative h-6 w-12 rounded-full bg-slate-600 transition
                                after:absolute after:left-0.5 after:top-0.5 after:h-5 after:w-5 after:rounded-full after:bg-white after:transition
                                peer-checked:bg-emerald-400 peer-checked:after:translate-x-6
                                peer-focus:ring-2 peer-focus:ring-emerald-300">
                            </span>

                            <span class="text-sm text-slate-300">Discord</span>
                        </label>
                    </div>
                </div>
                ...
                <div class="lg:col-span-6">
                    <div class="h-full pt-6">
                        <button
                                type="submit"
                                class="w-full h-[42px] rounded-lg bg-emerald-400 px-4 py-2 font-bold text-slate-900 hover:bg-emerald-300 transition"
                        >
                            Ajouter
                        </button>
                    </div>
    ...
    <section class="bg-slate-800 rounded-xl p-5">
        ...
        <div class="space-y-4">
            <?php foreach ($endpoints as $endpoint): ?>
                ...
                $updateFormId = 'endpoint-update-' . $id;
                ?>
                ...
                <article class="rounded-xl border <?= e($endpointCardClass) ?> p-4">
                    <form
                            id="<?= e($updateFormId) ?>"
                            method="post"
                            action="/admin/endpoints/<?= e($id) ?>/update"
                            class="space-y-4"
                    >
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
                            <div class="lg:col-span-2">
                                ...
                                <input
                                        ...
                                        required
                                        class="w-full h-[42px] rounded-lg bg-slate-950 border border-slate-700 px-3 py-2 text-slate-100 outline-none focus:border-emerald-400"
                                >
                            </div>
                            ...
                            <div class="lg:col-span-2">
                                ...
                                <select
                                        ...
                                        class="w-full h-[42px] rounded-lg bg-slate-950 border border-slate-700 px-3 py-2 text-slate-100 outline-none focus:border-emerald-400"
                                >
                                    <option value="seconds" <?= selectedUnit($endpoint->getUptimeUnit(), 'seconds') ?>>
                                        Secondes
                                    </option>
                                    <option value="milliseconds" <?= selectedUnit($endpoint->getUptimeUnit(), 'milliseconds') ?>>
                                        Millisecondes
                                    </option>
                                </select>
                            </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
                            <div class="lg:col-span-2">
                                <div class="h-full pt-6 grid grid-cols-2 gap-2">
                                    <div class="flex items-center justify-center h-[42px] rounded-[2rem] <?= $isEnabled ? 'bg-emerald-400 text-slate-900' : 'bg-red-500 text-white' ?>">
                                        <span class="text-xs font-bold">
                                            <?= $isEnabled ? 'Actif' : 'Désactivé' ?>
                                        </span>
                                    </div>

                                    <label class="flex items-center justify-center h-[42px] rounded-lg cursor-pointer">
                                        <input
                                                type="checkbox"
                                                name="discord_notifications_enabled"
                            ...
                            <div class="lg:col-span-6">
                                <div class="h-full pt-6 grid grid-cols-1 sm:grid-cols-3 gap-2">
                                    <button
                                            type="submit"
                                            form="<?= e($updateFormId) ?>"
                                            class="w-full h-[42px] rounded-lg bg-emerald-400 px-4 py-2 text-sm font-bold text-slate-900 hover:bg-emerald-300 transition"
                                    >
                                        Enregistrer
                                    </button>
                                    ...
                                    <button
                                            type="submit"
                                            form="endpoint-toggle-<?= e($id) ?>"
                                            class="w-full h-[42px] rounded-lg bg-slate-700 px-4 py-2 text-sm font-bold text-slate-100 hover:bg-slate-600 transition"
                                    >
                                    ...
                                    <button
                                            type="submit"
                                            form="endpoint-delete-<?= e($id) ?>"
                                            class="w-full h-[42px] rounded-lg bg-red-500 px-4 py-2 text-sm font-bold text-white hover:bg-red-400 transition"
                                    >
                                        Supprimer
                                    </button>
                                </div>
                    ...
                    <form
                            id="endpoint-toggle-<?= e($id) ?>"
                            method="post"
                            action="/admin/endpoints/<?= e($id) ?>/toggle"
                    ></form>

                    <form
                            id="endpoint-delete-<?= e($id) ?>"
                            method="post"
                            action="/admin/endpoints/<?= e($id) ?>/delete"
                            onsubmit="return confirm('Supprimer cet endpoint et ses downtimes ?');"
                    ></form>
                </article>
            <?php endforeach; ?>
        </div>
            ...
            <div>
                ...
                <div class="relative">
                    <input
                            ...
                            required
                            minlength="8"
                            class="w-full h-[42px] rounded-lg bg-slate-900 border border-slate-700 px-3 py-2 pr-11 text-slate-100 outline-none focus:border-emerald-400"
                    >
                                ...
                                <form method="post" action="/admin/admins/<?= e($adminId) ?>/password" class="grid grid-cols-1 gap-2 sm:grid-cols-[1fr_auto]">
                                    <div class="relative">
                                        <input
                                                ...
                                                minlength="8"
                                                required
                                                class="w-full h-[42px] rounded-lg bg-slate-950 border border-slate-700 px-3 py-2 pr-11 text-sm text-slate-100 outline-none focus:border-emerald-400"
                                        >
                                    ...
                                    <button
                                            type="submit"
                                            class="w-full sm:w-auto h-[42px] rounded-lg bg-emerald-400 px-4 py-2 text-sm font-bold text-slate-900 hover:bg-emerald-300 transition whitespace-nowrap"
                                    >
                                        Modifier
                                    </button>
                                </form>
                            <?php else: ?>
                                <div class="rounded-lg bg-slate-950 border border-slate-700 px-3 py-2 text-sm text-slate-500">
                                    Mot de passe non modifiable
                                </div>
                            <?php endif; ?>
...
<footer class="mb-10">
    ...
    <p class="text-center text-slate-500 text-sm">
        <a
                target="_blank"
                href="https://www.linkedin.com/in/théo-van-de-walle"
                rel="noopener noreferrer"
                class="text-slate-500 hover:text-emerald-400 transition"
        >
            Developed by Théo Van de Walle
        </a>
    </p>
</footer>
